...
export default function SubpageNavBar({
  ...
  const iconColor = isDark ? "#ffffff" : "var(--color-on-surface)";
  ...
  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between p-4 shrink-0"
      style={{ backgroundColor: bgColor ?? "var(--color-background)" }}
    >
      {/* Left: Back Arrow */}
      <Link
        href={backHref}
        className="w-[48px] h-[48px] rounded-full flex items-center justify-center shrink-0 transition-colors"
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            className="w-[48px] h-[48px] rounded-full flex items-center justify-center transition-colors cursor-pointer"
            style={{ border: `1px solid ${borderColor}` }}
            onMouseEnter={(e) => {
              e.currentTarget.style.backgroundColor = hoverBg;
            }}
        ...
        <div
          ...
          style={{
            backgroundColor: isDark ? "rgba(255,255,255,0.24)" : "var(--color-surface-variant)",
            border: `1.5px solid ${isDark ? "rgba(255,255,255,0.24)" : "color-mix(in srgb, var(--color-on-brand) 8%, transparent)"}`,
          }}
        >
          <Image
