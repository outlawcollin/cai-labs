...
export default function PersonaPicker({
  ...
  // Build display items: Browse + selected (if any) + fill with defaults
  // Always exactly 4 items, no scroll
  const getDisplayItems = (): (Persona | null)[] => {
    const defaults = mockPersonas.slice(0, 3);

    if (selectedPersona) {
      // If selected is already in defaults, don't duplicate
      const isInDefaults = defaults.some(p => p.id === selectedPersona.id);
      if (isInDefaults) {
        return defaults;
      }
      // Insert selected at position 0, shift others
      return [selectedPersona, defaults[0], defaults[1]];
    }
